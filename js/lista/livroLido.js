...
function atualizaTabela (como, especial) {
	...
	$.ajax({
	    url: "cgi-bin/lista/livroLido.py",
	    type: "post",
	    datatype: "html",
...
function colocaNaTable () {
	...
	for (i = inicio; i < livros.length && i < limitador; i++) {
		if (fixarEditora) {
			...
			if (fixarPalavrasChave) {
				var palavras = $("#palavrasChaves").val().split (", ");
				for (j = 0; j < palavras.length; j++) {
					if (livros[i].preco.indexOf(palavras[j]) != -1) {
						help = true;
						break;
					}
				}
			}
		if (help && fixarPalavrasChave) {
			help = false;
		} else if (fixarPalavrasChave) {
			continue;
		}
...
function excluirLivro (livrinho) {
	...
	$.ajax({
	    url: "cgi-bin/exclusao/livroLido.py",
	    type: "post",
	    datatype: "html",
...
$("#fixarPalavrasChaves").click (function () {
	autor = $(this).text();
	if (autor == "Fixar") {
		$(this).text("Desfixar");
		fixarPalavrasChave = true;
	} else {
		$(this).text ("Fixar");
		fixarPalavrasChave = false;
	}
});
...
$("#palavrasChaves").keyup (function () {
	palavrasChaves = $(this).val ();
	if (palavrasChaves == "") {
		colocaNaTable();
		return;
	}
	colocaNaTableComFiltro (palavrasChaves, "palavrasChaves");
});
/*
...
function colocaNaTableComFiltro (filtro, tipo) {
	...
	for (i = 0; i < livros.length; i++) {
		...
		} else if (tipo == "palavrasChaves") {
			valor = livros[i].preco.toUpperCase ();
		}
		if (valor.indexOf (filtro.toUpperCase()) != -1 || (tipo == "palavrasChaves" && verificaPalavraChave(filtro, valor))) {
			if (fixarEditora) {
				if (livros[i].editora.indexOf ($("#nomeEditora").val()) == -1) {
					continue;
				}
...
function verificaPalavraChave (palavras, valor) {
		help = false;
		palavras = palavras.split (", ");
		for (j = 0; j < palavras.length; j++) {
			if (valor.indexOf(palavras[j].toUpperCase()) != -1 && palavras[j].toUpperCase() != "") {
				help = true;
				break;
			}
		}
