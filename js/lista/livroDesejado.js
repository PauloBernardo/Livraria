...
function atualizaTabela (como, especial) {
	...
	//console.log ("Merda é essa?");
	$.ajax({
	    url: "cgi-bin/lista/livroDesejado.py",
	    type: "post",
	    datatype: "html",
...
function excluirLivro (livrinho) {
	...
	var empresa =  $(tds[7]).text();
	//console.log (empresa,preco, livroData);
	$.ajax({
	    url: "cgi-bin/exclusao/livroDesejado.py",
	    type: "post",
	    datatype: "html",
	    data: "Idcliente="+id+"&preco="+preco+"&empresa="+empresa+"&livroData="+livroData,
	    success: function (html)
	    {
	        var params = $(html).filter(function(){ return $(this).is('p') });
	        var gerarOpcao = false;
