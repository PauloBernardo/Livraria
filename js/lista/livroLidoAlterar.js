...
$(document).on ("click", "img.alterar", function () {
	...
	$("#precoLivroComprado").val (data);
	$("#dataCompraLivro").val(preco);
...
$("#alterarLivroComprado").click (function () {
	var data = $("#precoLivroComprado").val ();
	var preco = $("#dataCompraLivro").val();
	var livro = $("#selectLivroComprado").val().split("-")[4];
	if (preco == "" || data == "" || livro == "") {
		alert ("Preencha os dados!");
		return;
	}
	console.log (data, preco, livro);
	$.ajax({
	    url: "cgi-bin/alterar/livroLido.py",
	    type: "post",
	    datatype: "html",
