...
def main():
    ...
    try:
        ...
        conn = sqlite3.connect("SQLITE/Livraria.db")
        cursor = conn.cursor()
        cursor.execute ("""SELECT preco, data, idlivro FROM livroComprado WHERE idcliente = ?""", a)
        for i in cursor.fetchall():
            if str(i[0]).find (str(c)) != -1 and str(i[1]) == str(d) and str(i[2]) == idlivro:
                display_error ("ERRO NOME")
                return
        cursor.execute ("""UPDATE livroComprado SET preco = ? , data = ? , idlivro = ? WHERE idcliente = ? and idlivro = ? and preco = ? and data = ?""", (c,d,idlivro, a, idLivroAntigo, precoAntigo, dataAntiga))
        cursor.execute ("""SELECT livro.titulo, autor.nome, autor.sobrenome, genero.nome, editora.nome, livro.edicao, livro.data, livroComprado.preco, livroComprado.data
 FROM livroComprado inner join livro on livro.idlivro = livroComprado.idlivro 
 ...
 where livroComprado.idcliente = ? and livroComprado.preco = ? and livroComprado.data = ? order by livro.titulo;
""", (a, c, d))
        for i in cursor.fetchall():
