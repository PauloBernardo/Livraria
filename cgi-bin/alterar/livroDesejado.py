...
def main():
    ...
    try:
        ...
        conn = sqlite3.connect("SQLITE/Livraria.db")
        cursor = conn.cursor()
        cursor.execute ("""SELECT preco, empresa, idlivro FROM livroDesejado WHERE idcliente = ?""", a)
        for i in cursor.fetchall():
            if str(i[0]).find (str(c)) != -1 and str(i[1]) == str(d) and str(i[2]) == idlivro:
                display_error ("ERRO NOME")
                return
        cursor.execute ("""UPDATE livroDesejado SET preco = ? , empresa = ? , idlivro = ? WHERE idcliente = ? and idlivro = ? and preco = ? and empresa = ?""", (c,d,idlivro, a, idLivroAntigo, precoAntigo, dataAntiga))
        cursor.execute ("""SELECT livro.titulo, autor.nome, autor.sobrenome, genero.nome, editora.nome, livro.edicao, livro.data, livroDesejado.preco, livroDesejado.empresa
 FROM livroDesejado inner join livro on livro.idlivro = livroDesejado.idlivro 
 ...
 where livroDesejado.idcliente = ? and livroDesejado.preco = ? and livroDesejado.empresa = ? order by livro.titulo;
""", (a, c, d))
        for i in cursor.fetchall():
