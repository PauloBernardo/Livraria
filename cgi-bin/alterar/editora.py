...
def main():
    ...
    try:
        ...
        c = form["nome"].value
        nomeAntigo = form["nomeAntigo"].value
        bolean = False
        conn = sqlite3.connect("SQLITE/Livraria.db")
        cursor = conn.cursor()
        cursor.execute ("""SELECT nome FROM editora WHERE idcliente = ?""", a)
        for i in cursor.fetchall():
            if str(i[0]) == str(c):
                display_error ("ERRO NOME")
                return
        cursor.execute ("""UPDATE editora SET nome = ? where idcliente = ? and nome = ?;""", (c, a, nomeAntigo))
        conn.commit ()
